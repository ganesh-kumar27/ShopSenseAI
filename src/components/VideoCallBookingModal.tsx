...
const VideoCallBookingModal: React.FC<VideoCallBookingModalProps> = ({ store, isOpen, onClose }) => {
  ...
  // Generate next 7 days for date selection (only business days)
  const availableDates = useMemo(() => {
    ...
    return dates.slice(0, 7); // Limit to 7 available days
  }, [store]);
  ...
  const availableTimeSlots = useMemo(() => {
    ...
    // Use video call hours if available, otherwise use store hours
    const videoHours = store.videoCallHours || selectedDateInfo.hours;
    
    // Parse hours (simplified - assumes format like "9:00 AM - 8:00 PM")
    const [openTime, closeTime] = videoHours.split(' - ');
    ...
    return slots;
  }, [selectedDate, availableDates, store.videoCallHours]);
  ...
  const handleBookVideoCall = () => {
    ...
    setTimeout(() => {
      const selectedDateInfo = availableDates.find(d => d.date === selectedDate);
      alert(`Video call booked successfully! 📹\n\nStore: ${store.name}\nDate: ${selectedDateInfo?.displayDate}\nTime: ${selectedTime}\nDuration: 15 minutes\nCustomer: ${customerName}\n\nYou will receive a video call link via SMS 5 minutes before your scheduled time. Please ensure you have a stable internet connection.`);
      onClose();
        ...
        <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between p-6 border-b">
            <h2 className="text-xl font-semibold text-gray-900 flex items-center">
              ...
              Book 15-Min Video Consultation
            </h2>
            <button
              onClick={onClose}
          ...
          <div className="p-6">
            {!isConfirmed ? (
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="mt-2 text-xs text-green-700 bg-green-100 px-2 py-1 rounded inline-block">
                    Video consultations available: {store.videoCallHours || 'During business hours'}
                  </div>
                </div>
                ...
                <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <h4 className="font-medium text-green-900 mb-1">Video Consultation Benefits</h4>
                      <ul className="text-sm text-green-800 space-y-1">
                        <li>• Get personalized styling advice from store experts</li>
                        <li>• See products up close through live video</li>
                        <li>• Ask questions about fit, fabric, and availability</li>
                        <li>• 15-minute focused consultation session</li>
                        <li>• Free service - no consultation charges</li>
                      </ul>
                    </div>
                  </div>
                </div>
              ...
              <div className="text-center py-8">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">Booking Your Video Call...</h3>
                <p className="text-gray-600">Please wait while we confirm your 15-minute video consultation</p>
              </div>
            )}
          </div>
        </div>
